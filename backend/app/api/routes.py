...
# Legacy function kept for reference/fallback
async def _analyze_videos_with_agents(videos: List[VideoData]) -> List[VideoAnalysis]:
    """
    Analyze videos using all LLM agents.
    """
    ...
    # Step 1: Run individual video analysis (density, redundancy, title)
    video_results = []
    for video in videos:
        logger.info(f"Analyzing video: {video.title[:50]}...")
        
        # Run density analysis
        density_result = await density_agent.analyze(
        ...
        redundancy_result = await redundancy_agent.analyze(
            transcript=video.transcript,
            title=video.title,
            duration_seconds=video.duration_seconds,
        )
        
        # Run title relevance analysis
        title_result = await title_agent.analyze(
            transcript=video.transcript,
            title=video.title,
            summary=density_result.get("summary", ""),
        )
        ...
        logger.info(f"Individual analysis complete: density={density_result['score']}, redundancy={redundancy_result['score']}, title={title_result['score']}")
    
    # Step 2: Run originality analysis (cross-video comparison)
    originality_input = [
        {
    ...
    originality_results = await originality_agent.analyze(videos=originality_input)
    
    # Step 3: Build final analysis objects
    for result in video_results:
        video = result["video"]
        originality = originality_results.get(video.youtube_id, {
            "score": 50,
            "unique_aspects": [],
            "common_with_others": [],
        })
        ...
        analysis = VideoAnalysis(
            ...
            title_relevance=TitleRelevanceResult(
                ...
                is_clickbait=result["title"]["is_clickbait"],
                explanation=result["title"]["explanation"],
            ),
            originality=OriginalityResult(
                score=originality["score"],
                unique_aspects=originality["unique_aspects"],
                common_with_others=originality["common_with_others"],
            ),
        )
        analyses.append(analysis)
